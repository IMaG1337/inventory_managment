...
conn = psycopg2.connect(host="localhost", database="inventory_management", user="magomed", password="12345678")
...
def detail_employee(employee):
    sql_employee = f"""
        select emp.surname, emp."name", emp.patronymic, room.floor, room."number", count(inv.inventory_number_uid_id)
        from mainapp_employee emp
        left join mainapp_inventorycard inv
        on inv.employee_uid_id = emp.uid
        left join mainapp_rooms room
        on room.uid = inv.room_uid_id 
        where emp.uid = '{employee}'
        group by (room.floor, room."number", emp.surname, emp."name", emp.patronymic);
    """
    with conn.cursor() as curs:
        curs.execute(sql_employee)
        result = curs.fetchall()
...
def check_id(user_id):
    sql_id = f"""
        select tg.uid  from telegram_chat_id tg
        where tg.chat_uid = '{user_id}';
    """
    with conn.cursor() as curs:
        curs.execute(sql_id)
        result = curs.fetchall()
...
def update_chat_id(data: list) -> bool:
    # data = [123456789, '79995553322']
    sql_update = f"""
